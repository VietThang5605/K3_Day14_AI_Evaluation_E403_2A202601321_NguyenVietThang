...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        avg = (sum(all_values) / len(all_values)) if all_values else 0.5
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Refine system prompt and intent router to ensure answers address questions directly")
        if categories.get("incomplete", 0) > 0 or categories.get("Low_completeness", 0) > 0:
            suggestions.append("Increase chunk size in RAG pipeline and add few-shot examples to improve completeness")
        if categories.get("off_topic", 0) > 0:
            suggestions.append("Improve domain intent classification and grounding guardrails")

        default_suggestions = [
            "Implement hallucination checker to filter unsupported claims",
            "Refine system prompt and intent router to ensure answers address questions directly",
